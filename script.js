...
class InventoryManager {
  constructor() {
    ...
    this.mongoIntegration = useMongoDBIntegration
      ? new MongoDBIntegration()
      : null;
  ...
  // Initialisation de la base de données (IndexedDB ou MongoDB)
  async initializeDB() {
    try {
      // Si l'utilisateur est connecté et que l'intégration MongoDB est disponible
      if (this.mongoIntegration && this.mongoIntegration.isAuthenticated()) {
        // Utiliser MongoDB
        await this.loadArticlesFromMongoDB();
      } else {
        // Utiliser IndexedDB
  ...
  // Chargement des articles depuis MongoDB
  async loadArticlesFromMongoDB() {
    try {
      const articles = await this.mongoIntegration.getArticles();
      this.articles = articles;
      this.filteredArticles = [...this.articles];
      return articles;
    } catch (error) {
      console.error(
        "Erreur lors du chargement des articles depuis MongoDB:",
        error
      );
  ...
  async addArticle() {
    ...
    try {
      ...
      if (this.mongoIntegration && this.mongoIntegration.isAuthenticated()) {
        ...
        await this.mongoIntegration.addArticle(article);
        await this.loadArticlesFromMongoDB();
      } else {
  ...
  async updateArticle() {
    ...
    try {
      ...
      if (this.mongoIntegration && this.mongoIntegration.isAuthenticated()) {
        ...
        await this.mongoIntegration.updateArticle(article);
        await this.loadArticlesFromMongoDB();
      } else {
        // Mettre à jour dans IndexedDB
        article.id = parseInt(id); // Convertir en nombre pour IndexedDB
  ...
  async deleteArticle(id) {
    try {
      ...
      if (this.mongoIntegration && this.mongoIntegration.isAuthenticated()) {
        ...
        await this.mongoIntegration.deleteArticle(id);
        await this.loadArticlesFromMongoDB();
      } else {
        // Supprimer de IndexedDB
        await this.deleteArticleFromDB(parseInt(id)); // Convertir en nombre pour IndexedDB
        await this.loadArticles();
      }
  ...
  filterArticles() {
    ...
    this.filteredArticles = this.articles.filter((article) => {
      ...
      if (statusFilter === "in-stock") {
        ...
        matchesStatus =
          article.quantite <= article.seuil_alerte && article.quantite > 0;
    ...
    if (this.filteredArticles.length === 0) {
      container.innerHTML = `
                <div class="empty-state">
                    ...
                    <p>${
                      this.articles.length === 0
                        ? "Commencez par ajouter vos premiers articles ci-dessus"
                ...
                <div class="inventory-item ${statusClass}">
                    <div class="item-info">
                        ...
                        <div class="item-details">
                            ...
                            <span>Ajouté: ${new Date(
                              article.date_creation ||
                                article.derniere_date_achat
                            ).toLocaleDateString()}</span>
                        </div>
                    </div>
                    <div class="item-actions">
                        <button class="btn btn-primary btn-small" onclick="inventoryManager.openEditModal(${
                          article.id
                        })">
                            <i class="fas fa-edit"></i> Modifier
                        </button>
                        <button class="btn btn-danger btn-small" onclick="inventoryManager.openDeleteModal(${
                          article.id
                        }, '${article.nom}')">
  ...
  updateStats() {
    ...
    const alertItems = this.articles.filter(
      (article) => article.quantite <= article.seuil_alerte
    ).length;
  ...
  checkSuggestions() {
    ...
    const articlesEnRupture = this.articles.filter(
      (article) => article.quantite <= article.seuil_alerte
    );
    ...
    suggestionsList.innerHTML = articlesEnRupture
      .map((article) => {
